...
### Notes on the 2026-08-28 kitcn 0.32.1 bump (0.27.3 → 0.32.1, 10 releases)

⚠️ **This one was taken as a straight version bump at the user's request — the
per-package `dist/` audit that every section above documents was NOT performed.**
Treat the "verified unchanged" inventories in the previous sections as stale
relative to this delta: nothing below claims the builder core, the React
decorator set, or the ORM index-pushdown logic is still byte-identical, because
nobody looked. Do that audit before relying on any of those invariants again —
`authed-crpc-proxy.ts`'s decorator list, `end-user-sessions.ts`'s
`orderBy` pushdown, and the `.refine()`-bypasses-the-normalizer rationale are
the three that would fail quietly.

**convex still could not move, for the same reason as the last two bumps.**
kitcn 0.32.1's `convex` peer is unchanged at `>=1.42 <1.45.0`, and convex
`latest` is 1.45.0 — one patch above the cap. All four `convex` pins stay at
`1.44.0`; this is a **kitcn-only** bump. `bun install` emitted no peer warning.

**`bun run codegen` was mandatory and produced a real diff — but no new files.**
`git status --untracked-files=all` stayed clean of additions under `generated/`,
so unlike 0.25.1 (`aggregate.ts`) and 0.25.5 (`procedure-names.gen.ts`) there is
nothing to `git add` by name. `generated/server.ts` still carries
`capabilities: [migrationCapability()]`. What did change:

- **Two generated internal procedures flipped `mutation` → `query`**:
  `generated/server:migrationStatus` and
  `generated/aggregate:aggregateBackfillStatus`. Both `*.runtime.ts` registries
  and `_generated/api.d.ts` moved with them. ⚠️ This is a **deploy-ordering**
  concern, not a compile one: the CLI resolves these by module path through
  `runBackendFunction`, so `kitcn migrate status` against a deployment still
  running 0.27.3-era bindings is calling a function whose type no longer
  matches. Deploy the regenerated bindings before running `migrate status` /
  `aggregate` subcommands against prod.
- `createOrm()` now takes `internalQuery` alongside `internalMutation` in both
  `generated/server.ts` and `generated/aggregate.ts` — the mechanical
  consequence of the above. Same class of trap as 0.25.0's
  `migrationCapability`: a stale generated tree still typechecks.
- `generated/auth.ts` (the disabled-auth-runtime stub) gained a `count` export.
  Unused here — kitcn's auth runtime is off in this repo.
- `_generated/dataModel.d.ts` picked up kitcn-internal table changes only: a
  `deletionStack` column + `by_started_at` index on the auto-injected aggregate
  tables, and a nullability tweak on `aggregate_rank_node.items`. This repo
  declares no `aggregateIndex(...)`/`rankIndex(...)`, so none of it is reachable.

`shared/api.ts` regenerated byte-identical, so the frontend type surface is
untouched — which is consistent with zero hand-written changes being needed.

Gates after the bump: `oxfmt` + `oxlint --type-aware --fix` clean, `typecheck`
5/5, `build` 2/2 (both apps prerender `/` → 200). The `Request.cf` /
`TimeoutError` lines in the web build are miniflare failing to fetch
