...
Inside the procedure, `ctx.user` is typed `IdentityUser` (id, username, name, **role**) and `ctx.session` is `IdentitySession` (`{ id, lastSeenAt }`). There is no separate `ctx.userId` — use `ctx.user.id` instead. Identity comes from the **session token**: the builders merge `z.object({ sessionToken })` into the input, and the middleware's `resolveSession(ctx, token, roles)` looks the token up in the `session` table, rejecting a missing session, one whose `status` is no longer `active` (signed out / admin-revoked), or an expired one (`UNAUTHORIZED`), or a role outside the allowed tuple (`FORBIDDEN`) with a Chinese message before the body runs. It never touches `ctx.auth` — there is no JWT. See [auth](auth.md) for the full model (no Better Auth) and [session audit](feature-session-audit.md) for the status machine.
