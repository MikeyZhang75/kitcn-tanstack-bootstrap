...
- `apps/dashboard` — Second TanStack Start app, cloned from `apps/web` to serve admin users; deployed to Cloudflare Workers (worker name in `apps/dashboard/wrangler.jsonc`) at e.g. `https://dash.example.com`. Session gate requires `role === "admin"`. Currently hosts `/` (dashboard stub) and `/invitations` (invitation-code management) under `_authenticated`. Shares the same Convex backend and custom session-token auth as `apps/web`; the two differ only in which role the gate accepts and which routes render.
